...
- **桌面端：每个地址一个独立顶层窗口**：点击地址在**新的顶层窗口**中打开该 DeepSeek Harness 页面
  （同一地址再次点击只聚焦已打开的窗口，不重复打开、不重载页面）。
- **不使用 iframe**：iframe 里页面属于「第三方上下文」，DSH 的登录状态会被
  WebView 的跟踪防护拦截，dsh-LAN 的局域网口令页会反复闪烁、无法输入口令。
  桌面端改用独立顶层窗口、移动端改用本窗口顶层导航后，口令只需输入一次，行为与系统浏览器一致。
- **Cookie 与存储**：由系统 WebView 原生管理并持久化
  （Windows WebView2、Android WebView、iOS WKWebView）：重启 App 后保留，与系统浏览器数据隔离；
  桌面端每个地址窗口（源）各自独立。
- **页面实例常驻**：桌面端窗口存活期间页面不会自动重载（SPA 状态/滚动位置不丢）；
  需要重新加载时关闭该地址窗口再打开即可。
...
│   │   └── windows.ts            # 打开地址（桌面：顶层窗口 / 移动：本窗口导航）
│   ├── components/
