...
//   导航到该地址（`window.location.assign`）。顶层导航同样是第一方上下文，Cookie /
//   登录状态与系统浏览器一致；返回靠系统返回键 / 返回手势 —— Tauri 的 AppPlugin 会在
//   `canGoBack()` 为真时执行 `webView.goBack()`。因此**不要**在中枢页注册
//   `back-button` 事件监听：一旦注册，返回键只会派发事件而不回退历史，等于把移动端
//   唯一的返回路径掐断。
 ...
 * - `navigated`：移动端在**本窗口内**导航到了该地址——导航会卸载中枢页，
 *   调用方不要依赖导航之后的 UI 状态；
 ...
 * 本窗口内顶层导航：页面留在 App 内，返回靠系统返回键（回到地址中枢）。
 *
 * @param address - 展示形式的地址（如 `192.168.1.5:3080`）。
